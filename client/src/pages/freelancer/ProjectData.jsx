...
const ProjectData = () => {
  ...
  const fetchProject = async(id) =>{
    await axios.get(`http://localhost:6001/fetchProject/${id}`).then(
      (response)=>{
    ...
    const handleBidding = async() =>{
      ...
      await axios.post("http://localhost:6001/makeBid", {clientId, freelancerId, projectId, proposal, bidAmount, estimatedTime}).then(
        (response)=>{
            setProposal('');
            setBidAmount(0);
            setEstimatedTime(0);
    ...
    const handleProjectSubmission = async() =>{
      ...
      await axios.post("http://localhost:6001/submitProject", {clientId, freelancerId, projectId, projectLink, manualLink, submissionDescription}).then(
        (response)=>{
            setProjectLink('');
            setManualLink('');
    ...
    const fetchChats = async() =>{
      await axios.get(`http://localhost:6001/fetchChats/${params['id']}`).then(
        (response) =>{
